...
def load_data_from_json_and_associate_images(json_path: str, image_dir: str) -> List[Dict[str, Any]]:
    ...
    for item_index, item in enumerate(json_data):
        ...
        if image_dir and os.path.isdir(image_dir):
            ...
            func_logger.warning(f"Provided images directory '{image_dir}' is not a valid directory. Cannot associate images.")
            image_dir_warning_issued = True
        elif not image_dir:
             func_logger.debug(f"No images directory provided (image_dir is None or empty), skipping images association.")
    ...
    func_logger.info(f"Found and associated images files for {found_images_count} valid documents.")

    if len(documents) > 0 and found_images_count == 0 and image_dir and os.path.isdir(image_dir):
         func_logger.info(f"NOTE: No images files were found in '{image_dir}' matching the document IDs.")
         func_logger.info(f"      Check if images filenames exactly match document IDs (e.g., 'item01.png' for 'name': 'item01').")

    func_logger.info("--- Data loading and images association finished ---")
    return documents
...
class MultimodalEncoder:
    ...
    def encode(self, text: Optional[str] = None, image_path: Optional[str] = None) -> Dict[str, Optional[np.ndarray]]:
        """
        Encodes input text and/or images path into normalized feature vectors.

        Args:
            text (Optional[str]): Text to encode. Skipped if None or empty.
            image_path (Optional[str]): Path to the images file to encode. Skipped if None or invalid.

        Returns:
            Dict[str, Optional[np.ndarray]]: Dictionary containing 'text_vector', 'image_vector',
                                             and 'mean_vector' (if both text and images are encoded).
        ...
        if not is_text_valid and not is_image_path_valid:
            self.logger.error("Encoding Error: Must provide valid text or images path.")
        ...
        with torch.no_grad(): # Disable gradient calculation for inference
            ...
            if is_image_path_valid:
                self.logger.debug(f"Encoding images: '{image_path}'")
                try:
                    # Check if images exists before trying to open
                    ...
                    image_pil.close() # Close the images file handle

                except FileNotFoundError:
                    self.logger.warning(f"Image encoding skipped: File not found at '{image_path}'.")
                    image_vector = None
                except UnidentifiedImageError:
                    self.logger.error(f"Image encoding error: Cannot identify or open images file '{image_path}'. It might be corrupt or unsupported.")
                    image_vector = None
                except Exception as e:
                    self.logger.error(f"Error encoding images '{image_path}'. Details: {e}", exc_info=False)
                    image_vector = None

            # --- Calculate Mean Vector ---
            if text_vector is not None and image_vector is not None:
                self.logger.debug("Calculating mean vector for text and images...")
                try:
...
class Indexer:
    ...
    - Storing document metadata (ID, text, images path) in an SQLite database.
    - Building and managing separate Faiss indices for text, images, and mean vectors.
    ...
    def __init__(self,
        ...
        Args:
            ...
            faiss_image_index_path (str): Path for the images vector Faiss index file.
    ...
    def index_documents(self, documents: List[Dict[str, Any]]):
        ...
        try:
            ...
            for i, doc_data in enumerate(documents):
                ...
                # At least text or a valid images path should ideally exist for meaningful encoding,
                ...
                try:
                    ...
                    if encoded_data and (encoded_data.get('text_vector') is not None or
                        ...
                        self.logger.debug(f"Document '{doc_id_str}' encoded successfully (at least one vector generated).")
                         ...
                         # This case means encode ran but produced no vectors (e.g., images load failed silently in encode, or text was empty and no images provided)
                ...
                try:
                    cursor.execute(
                        ...
                        (doc_id_str, text_content_str, image_file_path_str) # Use potentially empty text, potentially None images path
                    )
                ...
                if internal_id is not None and encoded_data is not None:
                    ...
                    if encoded_data.get('image_vector') is not None:
                        ...
                        at_least_one_vector_added_for_doc = True
                        self.logger.debug(f"  Prepared images vector for doc '{doc_id_str}' (internal_id: {internal_id}) for batch add.")
    ...
    def save_indices(self):
        """
        Saves all three Faiss indices (text, images, mean) to their respective files.
...
class Retriever:
    """
    Handles the retrieval process based on user queries (text, images, or multimodal).
    Uses the MultimodalEncoder (via Indexer) to vectorize queries and searches the
    appropriate Faiss index (text, images, or mean) provided by the Indexer.
    ...
    def retrieve(self, query: Union[str, Dict[str, str]], k: int = 5) -> List[Dict[str, Any]]:
        ...
        Args:
            query (Union[str, Dict[str, str]]): The user query (text string, or dict for images/multimodal).
        ...
        if isinstance(query, str):
            ...
            if query_text and query_image_path:
                # Validate images path existence for multimodal query here? Or let encoder handle it? Let encoder handle.
                query_type = "multimodal"
                self.logger.info(f"    Query type: {query_type}")
                self.logger.info(f"    Query text part: '{query_text[:50]}...'")
                self.logger.info(f"    Query images part: '{os.path.basename(query_image_path)}'")
            elif query_image_path:
                # Validate images path for pure images query *before* encoding
                if os.path.exists(query_image_path) and os.path.isfile(query_image_path):
                    query_type = "pure_image"
                    self.logger.info(f"    Query type: {query_type}")
                    self.logger.info(f"    Query images path: '{os.path.basename(query_image_path)}' (File exists)")
                else:
                    self.logger.error(f"Query Error: Pure images query path invalid/not found: '{query_image_path}'")
                    return []
            elif query_text:
                query_type = "pure_text"
        ...
        if query_type == "pure_text":
            ...
            if query_image_vec is not None and image_ntotal > 0:
                ...
                self.logger.info(f"    Strategy: Use images vector, search in {selected_index_name} ({image_ntotal} vectors).")
                strategy_applied = True
            else:
...
class Generator:
    ...
    def _build_messages(self, query: str, context: List[Dict[str, Any]]) -> List[Dict[str, str]]:
        ...
        system_prompt_instructions = [
            ...
            "3.  **Understanding Image Content:** You cannot directly 'see' or analyze images files. Your understanding of any associated images MUST come *only* from the textual descriptions provided within the 'Reference Documents' for that images, or the images's filename if mentioned. Never claim visual perception.",
            "4.  **Answering Questions About Images:**",
            "    - If a question refers to an images (by filename or description), meticulously search the *textual content* of the 'Reference Documents' for relevant descriptions.",
        ...
        if not context:
            ...
            for i, doc in enumerate(context):
                ...
                image_info = f"Associated Image Filename: '{image_filename}'" if image_filename else "No associated images file mentioned."
